...
impl DaemonWithProxy {
    async fn spawn(proxy_args: ProxyArgs, kumod_env: Vec<(String, String)>) -> anyhow::Result<Self> {
...
async fn proxy_with_lua_config() -> anyhow::Result<()> {
    let mut daemon = ProxyDaemon::spawn(ProxyArgs {
        proxy_config: Some("proxy_init.lua".to_string()),
        env: vec![],
    })
    .await?;
    ...
    let mut daemon = DaemonWithProxy::spawn(
        ProxyArgs {
            ...
            env: vec![
                ...
                ("KUMO_PROXY_AUTH_USERNAME".to_string(), "testuser".to_string()),
                ("KUMO_PROXY_AUTH_PASSWORD".to_string(), "testpass".to_string()),
            ],
        },
        vec![
...
async fn proxy_end_to_end_auth_negative() -> anyhow::Result<()> {
    ...
    // Give it a moment to attempt delivery
    tokio::time::sleep(Duration::from_secs(2)).await;

    // Verify NO messages were delivered (auth should have failed)
    let messages = daemon.kumod.extract_maildir_messages()?;
    anyhow::ensure!(
        messages.is_empty(),
        "expected 0 messages delivered with wrong auth, got {}",
        messages.len()
    );
...
async fn proxy_require_auth_rejects_noauth_client() -> anyhow::Result<()> {
    ...
    anyhow::ensure!(
        messages.is_empty(),
        "expected 0 messages when require_auth rejects client, got {}",
        messages.len()
    );
