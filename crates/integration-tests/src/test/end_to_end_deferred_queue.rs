...
/// This test enables deferred_queue = true in the listener
/// and verifies that we do delivery the message through
/// to the sink.
#[tokio::test]
async fn end_to_end_deferred_queue() -> anyhow::Result<()> {
    let mut daemon =
        DaemonWithMaildir::start_with_env(vec![("KUMOD_DEFERRED_SMTP_SERVER_MSG_INJECT", "1")])
            .await
