...
### NETWORK
The bandwidth above is important because AWS EC2 network interfaces typically [support 5Gbps](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-instance-network-bandwidth.html) at the low end.  This means you don't need to stress about adding any network capacity for this build.
...
Logs will also consume drive space.  The amount of space is highly dependent on your specific configuration, but based on the default sample policy, each log line will consume about 1KB and there may be an average of 5 log lines per message.  In this example we are going to assume logging at a rate of 5KB per message and then double it for safety. _We can talk more about reducing log space requirements ina later section._
